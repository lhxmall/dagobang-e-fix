...
import { isO1LaunchpadPlatform, normalizeLaunchpadPlatform } from '@/constants/launchpad';
...
export class TokenO1Service {
  static async getTokenInfo(
    ...
    if (consoleLogsEnabled) {
      console.log('[o1.tokenInfo.rpc.start]', {
        chainId,
    ...
    const state = await getO1LaunchState(tokenAddress as Address, {
      poolId: pickPoolId(seed),
    });
    if (!state) return null;

    const quoteSymbol = await readO1QuoteSymbol(state.quoteToken);
    const seedPlatform = normalizeLaunchpadPlatform(seed?.launchpad_platform || seed?.launchpad);
    const launchpad = seedPlatform && isO1LaunchpadPlatform(seedPlatform)
      ? seedPlatform
      : (state.suiteId.includes('rwa') ? 'o1_rwa' : 'o1');
    ...
    if (consoleLogsEnabled) {
      console.log('[o1.tokenInfo.rpc.done]', {
        chainId,
        tokenAddress: tokenAddress.toLowerCase(),
